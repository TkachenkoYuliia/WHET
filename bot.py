...
def cmd_start(message):
    ...
    bot.send_message(
        ...
        "Напиши перелік продуктів через кому (кількість не обов'язкова).\n"
        "Наприклад:\n"
        "<code>яйця, молоко, борошно, помідори</code>",
...
def cmd_help(message):
    bot.send_message(
        message.chat.id,
        "ℹ️ <b>Допомога</b>\n\n"
        "1️⃣ Напиши перелік продуктів у холодильнику (через кому):\n"
        "   • <code>яйця, молоко, борошно, помідори</code>\n\n"
...
def btn_add_products(message):
    ...
    if s["products"]:
        parts = []
        for k, (q, u) in s["products"].items():
            parts.append(k if q >= 999 else f"{k} {q} {u}")
        products_text = "\n\nПоточний список:\n" + ", ".join(parts)
    bot.send_message(
        message.chat.id,
        "📋 Напиши перелік продуктів через кому.\n\n"
        "Приклад: <code>яйця, молоко, борошно, помідори</code>" + products_text,
    )
...
def handle_text(message):
    ...
    if s["step"] == "waiting_products" or s["step"] is None:
        ...
        if not products:
            bot.send_message(
                message.chat.id,
                "Не вдалося розпізнати продукти. Спробуй:\n"
                "<code>яйця, молоко, борошно, помідори</code>",
            )
            return
        ...
        def fmt(p):
            ...
            if q >= 999:
                return f"• {k}"
            return f"• {k}: {q} {u}"
        text = "✅ Продукти збережено:\n" + "\n".join(fmt(x) for x in products.items())
    ...
    if products:
        ...
        bot.send_message(
            message.chat.id,
            "Напиши перелік продуктів (напр. <code>яйця, молоко, борошно</code>) "
